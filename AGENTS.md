...
### 🔴 强制禁止：主页/一级页面"千篇一律、未细化的信息"（用户 2026-08-16 强约束）

主页（一级页面）**禁止出现对普通阅读者无意义、且在所有功能间雷同未细化的信息**。判断标准不是"该字段是否存在"，而是**它是否为当前功能定向输出**：

- ❌ 禁止「证据 N 条 · 跨市场 / 竞品 / 用户 / 战略四维交叉」这类**每个功能卡片一字不差**的证据统计套话——因为未细化、无信息增量。
- ❌ 禁止「功能补齐」这类**所有 TOP5 都相同**的模板化定位标签。
- ✅ 若保留证据/场景等字段，必须**针对当前功能写入具体内容**（如"续航 · 876 人 PK / 搜狐实测不足半天"），使每个功能的该字段**各不相同、有实际信息**。
- ✅ 主页信息优先用各功能专属的**用户场景 / 痛点 / 根因 / 差异化**定向呈现。
- 功能总览/候选功能给**简易 PRD 内容**（价值/背景来源/目标用户/方案），**不得**堆叠证据链接作为一级呈现；候选功能一级界面不用超链接形式。

核心判断：**同一字段在不同功能卡片上必须是不同的、具体的、可读的；凡所有功能共用一套文案即为违规**。违反即视为不合格，需重做。
