...
// ---------- 7. 派生 GTMS（补 northStar/first100） ----------
const GTMS = {};
for (const s of scores) {
  if (!s.gtm) continue;
  GTMS[s.ideaId] = {
    audience: s.gtm.audience || '目标客群',
    channel: s.gtm.channel || '—',
    pricing: s.gtm.pricing || '—',
    entry: s.gtm.entry || '—',
    northStar: '目标客群渗透率 ≥ 5%（发布后 12 个月）',
    first100: '雷鸟社区/粉丝群 100 人内测',
  };
}
